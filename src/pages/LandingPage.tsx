    ...
    return parts.map((part, i) => {
      ...
      if (isCode)
        return (
          <span
            key={i}
            className="inline-block ml-1 bg-primary-800 text-white text-xs font-semibold px-2 py-0.5 rounded"
          >
            {part}
          </span>
        );
    ...
    <div className="min-h-screen bg-white text-gray-900">
      ...
      <header className="sticky top-0 z-50 px-4 pt-3 pb-2">
        <div className="max-w-6xl mx-auto rounded-2xl bg-white shadow-lg border border-gray-100 overflow-hidden bg-gradient-to-b from-white to-sky-50/50">
          <div className="h-14 md:h-16 flex items-center justify-between px-5 md:px-8">
            <Link
              to="/"
              className="flex items-center gap-2 font-bold text-xl text-primary-800"
            >
              <span
                className="w-9 h-9 rounded-full bg-primary-500 flex items-center justify-center text-white"
                aria-hidden
              >
                <svg
                  className="w-5 h-5"
        ...
        <div className="max-w-6xl mx-auto px-4 py-16 md:py-24 grid md:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <div className="mt-8 flex flex-wrap gap-4">
              <Link
                to="/signup"
                className="bg-primary-600 text-white px-8 py-4 rounded-xl font-semibold hover:bg-primary-700 shadow-lg"
              >
                {c("HeroCtaPrimary", data)}
              </Link>
              <a
                href={c("HeroCtaSecondaryUrl", data)}
                className="bg-white text-primary-600 border-2 border-primary-600 px-8 py-4 rounded-xl font-semibold hover:bg-primary-50"
              >
                {c("HeroCtaSecondary", data)}
              </a>
            </div>
            <p className="mt-6 text-gray-600">
              Join {HappyEngineers} happy engineers.
            </p>
          </div>
          <div className="relative">
            <div className="aspect-video bg-primary-700/20 rounded-2xl flex items-center justify-center text-primary-800 font-medium">
              Video / Demo
            </div>
            <div className="mt-4 p-4 bg-white rounded-xl shadow border border-gray-100 text-sm text-gray-600">
              AI chat preview
            </div>
          </div>
        </div>
      ...
      <section id="features" className="py-16 md:py-24 bg-white">
        <div className="max-w-6xl mx-auto px-4 text-center">
          ...
          <div className="mt-12 p-8 md:p-12 bg-primary-600 rounded-3xl text-white text-left max-w-4xl mx-auto">
            ...
            <Link
              to="/signup"
              className="inline-block mt-6 bg-white text-primary-600 px-8 py-3 rounded-xl font-semibold hover:bg-primary-50"
            >
              {c("GetStartedFree", data)}
            </Link>
            <div className="mt-8 flex flex-wrap gap-4">
        ...
        <div className="max-w-6xl mx-auto px-4">
          ...
          <div className="mt-12 grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {data.featureCardsHowItHelps.map((card, i) => (
              ...
              >
                <div className="bg-primary-100 text-primary-800 w-12 h-12 rounded-xl flex items-center justify-center text-xl font-bold">
                  {card.iconName ? card.iconName[0] : "?"}
                </div>
                ...
                {card.extraData &&
                  (() => {
                    try {
                      const ex = JSON.parse(card.extraData) as {
                        score?: string;
                        scoreLabel?: string;
                      };
                      if (ex.score)
      ...
      {data.testimonials.length > 0 && (
        <section className="py-16 md:py-24 bg-white">
          <div className="max-w-6xl mx-auto px-4">
            <h2 className="text-3xl md:text-4xl font-bold text-gray-900 text-center">
              {c("TestimonialsTitle", data)}
            </h2>
            <div className="mt-12 flex gap-6 overflow-x-auto pb-4">
              {data.testimonials.map((t, i) => (
                <div
                  key={i}
                  className="min-w-[300px] max-w-[340px] bg-gray-50 rounded-2xl p-6 border border-gray-100"
                >
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-full bg-primary-200 flex items-center justify-center text-primary-800 font-bold">
                      {t.name[0]}
                    </div>
                    <div>
        ...
        <div className="max-w-6xl mx-auto px-4 flex flex-col md:flex-row items-center justify-between gap-8">
          <div>
            <h2 className="text-2xl md:text-3xl font-bold text-white">
              {data.stats?.QuestionsCount ?? "500+"} Questions in{" "}
              {data.stats?.TopicsCount ?? "20+"} Topics
            </h2>
            <p className="mt-2 text-primary-100">
              No more searching for questions. Practice what matters.
            </p>
            <Link
              to="/signup"
              className="inline-block mt-6 bg-white text-primary-600 px-8 py-3 rounded-xl font-semibold hover:bg-primary-50"
      ...
      <section className="py-16 bg-primary-700">
        <div className="max-w-6xl mx-auto px-4 flex flex-col md:flex-row items-center justify-between gap-8">
          <div>
            <h2 className="text-2xl md:text-3xl font-bold text-white">
              {c("DominateTitle", data)}
            </h2>
            <p className="mt-2 text-primary-100">
              {c("DominateSubtitle", data)}
            </p>
            <Link
              to="/signup"
              className="inline-block mt-6 bg-white text-primary-600 px-8 py-3 rounded-xl font-semibold hover:bg-primary-50"
          ...
          <div className="w-48 h-48 bg-white/20 rounded-2xl flex items-center justify-center text-white text-4xl">
            😊
          </div>
        </div>
      </section>
      ...
      <section
        id="pricing"
        className="py-16 md:py-24 bg-gradient-to-b from-primary-800 to-primary-900"
      >
        <div className="max-w-6xl mx-auto px-4 text-center">
          ...
          <div className="mt-12 flex justify-center gap-4 mb-12">
            <span className="px-4 py-2 rounded-lg bg-white/10 text-white text-sm">
              {c("PricingMonthlyLabel", data)}
            </span>
            <span className="px-4 py-2 rounded-lg bg-primary-500 text-white text-sm">
              {c("PricingYearlyLabel", data)}
            </span>
          </div>
              ...
              >
                ...
                {plan.yearlyDiscountPercent != null && (
                  <p className="text-sm text-primary-700 mt-1">
                    Save {plan.yearlyDiscountPercent}% by paying yearly
                  </p>
                )}
      ...
      <section className="py-12 bg-white">
        <div className="max-w-6xl mx-auto px-4 text-center">
          ...
          <button
            type="button"
            className="mt-4 bg-primary-600 text-white px-8 py-3 rounded-xl font-semibold hover:bg-primary-700"
          >
            {c("TrustCtaText", data)}
          </button>
      ...
      <section className="py-16 bg-white">
        <div className="max-w-6xl mx-auto px-4 grid md:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <Link
              to="/signup"
              className="inline-block mt-6 bg-primary-600 text-white px-8 py-3 rounded-xl font-semibold hover:bg-primary-700"
            >
              {c("GetStartedFree", data)}
            </Link>
          </div>
          <div className="grid grid-cols-2 gap-4">
            {[1, 2, 3, 4].map((i) => (
              <div
                key={i}
                className="aspect-square bg-primary-100 rounded-xl"
              />
            ))}
          </div>
        </div>
                ...
                <div key={cat}>
                  ...
                  <ul className="space-y-2">
                    {data.footerLinks
                      ...
                      .map((f, i) => (
                        <li key={i}>
                          <a href={f.url} className="hover:text-white">
                            {f.label}
                          </a>
                        </li>
                      ))}
            ...
            <div className="w-full md:max-w-sm">
              ...
              >
                ...
                <button
                  ...
                  className="bg-primary-500 text-white px-6 py-2 rounded-lg font-semibold hover:bg-primary-600 disabled:opacity-50"
                >
                  Subscribe
                </button>
              </form>
