...
import { callSessionsApi, type CallSessionDto, type CallSessionMessageDto } from '../api/callSessions'
...
export default function DashboardPage() {
  ...
  const [resumeModalStep, setResumeModalStep] = useState<'options' | 'upload-pdf'>('options')
  ...
  const [createSessionMode, setCreateSessionMode] = useState<'create' | 'update'>('create')
  ...
  const [createdSessionIdForLaunch, setCreatedSessionIdForLaunch] = useState<string | null>(null)
  ...
  const [sessionForPlatformModal, setSessionForPlatformModal] = useState<CallSessionDto | null>(null)
  const [conversationModalSession, setConversationModalSession] = useState<CallSessionDto | null>(null)
  const [conversationMessages, setConversationMessages] = useState<CallSessionMessageDto[]>([])
  const [conversationMessagesLoading, setConversationMessagesLoading] = useState(false)
  const [buyCreditsTab, setBuyCreditsTab] = useState<'credits' | 'subscription' | 'lifetime'>('credits')
  ...
  useEffect(() => {
    ...
    } else if (pathname.startsWith('/dashboard/buyCredits') || pathname.startsWith('/dashboard/purchase')) {
  ...
  useEffect(() => {
    const state = location.state as { openCreateSessionModal?: 'free' | 'paid' } | null
    if (location.pathname.startsWith('/dashboard/call-sessions') && state?.openCreateSessionModal) {
      setCreateSessionIsPaid(state.openCreateSessionModal === 'paid')
      resumesApi.list().then(setResumesList).catch(() => setResumesList([]))
  ...
  useEffect(() => {
    ...
    callSessionsApi.list(sessionsPage, 10)
      .then((r) => {
        setSessionsList(r.items)
        setSessionsTotal(r.totalCount)
      })
      .catch(() => { setSessionsList([]); setSessionsTotal(0); })
  ...
  useEffect(() => {
    ...
    callSessionsApi.getMessages(conversationModalSession.id)
  ...
  useEffect(() => {
    ...
    const t = setInterval(() => {
      ...
      if (i >= steps.length) {
        ...
        setResumeTitle((prev) => (prev === 'My Resume' ? resumeFile.name : prev))
      }
    }, 200)
  ...
  useEffect(() => {
    ...
    resumesApi.getFileUrl(previewResumeId)
  ...
  const openUpdateSessionModal = (s: CallSessionDto) => {
    ...
    setCreateSessionForm((f) => ({
      ...
      extraContext: s.extraContext || 'Keep answers to the point and with a coding example',
  ...
  const finishChoosePlatform = () => {
    ...
    setActiveNav('call-sessions')
    setSessionsPage(1)
    callSessionsApi.list(1, 10).then((r) => {
      setSessionsList(r.items)
      setSessionsTotal(r.totalCount)
    }).catch(() => {})
  }
  ...
  const launchDesktopApp = (sessionId: string, resumeId?: string | null) => {
    ...
    const apiBase = (import.meta.env.VITE_API_URL || import.meta.env.VITE_API_PUBLIC_URL || 'http://localhost:5050/api').replace(/\/?$/, '/')
  ...
  const handleUploadResumeSubmit = async () => {
    ...
    try {
      ...
      const message = (e as { response?: { data?: { message?: string } } })?.response?.data?.message ?? 'Upload failed. Please try again.'
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <aside className={`${sidebarOpen ? 'w-64' : 'w-20'} sticky top-0 h-screen bg-white border-r border-gray-200 flex flex-col transition-all duration-300 shrink-0`}>
        {/* Logo */}
        <div className="p-4 border-b border-gray-100 flex items-center gap-2.5">
          ...
          {sidebarOpen && <span className="font-semibold text-gray-800 text-base">OrioAI</span>}
        </div>
        ...
        <nav className="flex-1 py-3 overflow-y-auto">
          {mainNavItems.map(({ id, label, Icon }) => (
            <button
              ...
              type="button"
              onClick={() => {
                if (id === 'call-sessions') navigate('/dashboard/call-sessions')
                else if (id === 'cvs') navigate('/dashboard/cvs')
                else if (id === 'documents') navigate('/dashboard/documents')
          ...
          <button type="button" className="w-full flex items-center gap-3 px-4 py-2.5 text-left text-gray-600 hover:bg-gray-50 hover:text-gray-900 transition">
            ...
            {sidebarOpen && (
              <>
                <span className="text-sm font-medium flex-1">Download Desktop App</span>
                <IconChevronDown />
              </>
            )}
          </button>
        ...
        {sidebarOpen && (
          <div className="p-4 border-t border-gray-100 space-y-4">
            <div className="rounded-xl bg-gray-50 border border-gray-100 p-4">
              <div className="flex items-center justify-between gap-2 mb-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-semibold text-gray-800 text-sm">Credits</span>
                  <button type="button" className="p-0.5 text-gray-400 hover:text-gray-600" aria-label="Info"><IconInfo /></button>
                </div>
                <button type="button" className="p-1 text-gray-400 hover:text-gray-600" aria-label="More"><IconMoreVertical /></button>
              </div>
              <p className="text-sm text-gray-600">
                You have <span className="inline-block px-2 py-0.5 rounded-md bg-primary-100 text-primary-700 font-semibold">2.5</span> credits.
              </p>
              <button
                type="button"
            ...
            >
              <span className="shrink-0"><IconUser /></span>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="bg-white border-b border-gray-200 h-14 flex items-center justify-between px-6 shrink-0">
          <div className="flex items-center gap-4">
            <button type="button" onClick={() => setSidebarOpen((o) => !o)} className="p-2 rounded-lg hover:bg-gray-100 lg:hidden">
            ...
            {previewResumeId ? (
              ...
              <h1 className="text-lg font-semibold text-gray-800">
                {mainNavItems.find((i) => i.id === activeNav)?.label ?? (activeNav === 'profile' ? 'Profile' : 'Home')}
              </h1>
            )}
          </div>
          <div className="flex items-center gap-3">
            {previewResumeId ? (
              ...
              <>
                <button
                  type="button"
                  onClick={() => navigate('/dashboard/call-sessions', { state: { openCreateSessionModal: 'free' } })}
          ...
          {previewResumeId ? (
            ...
            <div className="flex-1 flex flex-col min-h-0 bg-gray-100">
              ...
              <div className="flex items-center gap-3 px-4 py-2 bg-gray-700 text-gray-200 text-sm shrink-0">
                <button type="button" className="p-1.5 rounded hover:bg-gray-600" aria-label="Menu">
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" /></svg>
                </button>
                <span className="truncate max-w-[180px] text-gray-300" title={previewFileName}>
                  {previewFileName ? `${previewFileName.slice(0, 20)}${previewFileName.length > 20 ? '...' : ''}` : '…'}
                </span>
                <span className="text-gray-400 text-xs">1 / 1</span>
                <span className="text-gray-400">100%</span>
                <button type="button" className="p-1 rounded hover:bg-gray-600" aria-label="Zoom out">−</button>
                <button type="button" className="p-1 rounded hover:bg-gray-600" aria-label="Zoom in">+</button>
                <button type="button" className="p-1.5 rounded hover:bg-gray-600" aria-label="Rotate left">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6" /></svg>
                </button>
                <button type="button" className="p-1.5 rounded hover:bg-gray-600" aria-label="Rotate right">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 10h-10a8 8 0 00-8 8v2M21 10l-6 6m6-6l-6-6" /></svg>
                </button>
                {previewPdfUrl && (
                  <>
                    <a href={previewPdfUrl} download={previewFileName || 'resume.pdf'} className="p-1.5 rounded hover:bg-gray-600" aria-label="Download">
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>
                    </a>
                    <button type="button" onClick={() => window.print()} className="p-1.5 rounded hover:bg-gray-600" aria-label="Print">
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z" /></svg>
                    </button>
                  </>
                )}
                <button type="button" className="p-1.5 rounded hover:bg-gray-600 ml-auto" aria-label="More">
                  <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M12 8c1.1 0 2-.9 2-2s-.9-2-2-2-2 .9-2 2 .9 2 2 2zm0 2c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2zm0 6c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2z" /></svg>
                </button>
              </div>
              {/* PDF content */}
              <div className="flex-1 min-h-0 bg-gray-500 flex items-center justify-center p-4">
                {previewLoading && <div className="text-white">Loading PDF…</div>}
                {!previewLoading && previewPdfUrl && (
                  <iframe src={previewPdfUrl} title="Resume PDF" className="w-full h-full max-w-4xl bg-white rounded shadow-lg" />
                )}
                {!previewLoading && !previewPdfUrl && previewResumeId && <div className="text-white">Failed to load PDF.</div>}
              </div>
            </div>
          ) : activeNav === 'home' ? (
                ...
                {[
                  ...
                  <div key={i} className="flex items-stretch gap-2 flex-1 min-w-0">
                    <div className="flex-1 flex flex-col min-h-0 rounded-xl border border-gray-200 bg-white p-5 shadow-sm hover:shadow-md transition">
                      ...
                      <p className="text-sm text-gray-600 mt-2 flex-1 min-h-0">{card.desc}</p>
                      <button
                        type="button"
                        onClick={() =>
                          card.primary
                            ? navigate('/dashboard/call-sessions', { state: { openCreateSessionModal: 'paid' } })
                            : card.btn === 'Create Session'
                            ? navigate('/dashboard/call-sessions', { state: { openCreateSessionModal: 'free' } })
                            : card.id === 'cvs'
                    ...
                    {i < 3 && (
                      <span className="hidden lg:inline-flex items-center shrink-0 text-gray-500" aria-hidden>
            ...
            <div className="flex-1 overflow-auto bg-gray-50">
              <div className="bg-amber-100 text-amber-900 text-sm py-2 px-6 border-b border-amber-200">
                <span className="font-semibold">Special offer for IN India users:</span> Use code{' '}
                <span className="font-mono font-semibold bg-amber-200 px-1 rounded">INDIA25</span> for 25% off!{' '}
              ...
              <div className="max-w-5xl mx-auto py-8 px-4 space-y-10">
                <div className="text-center space-y-2">
                  <p className="text-xs font-semibold tracking-[0.2em] text-emerald-500">PRICING</p>
                  <h2 className="text-3xl font-extrabold text-gray-900">
                    Buy Credits or <span className="text-emerald-500">Go Unlimited ✨</span>
                  </h2>
                </div>
                <div className="flex justify-center">
                  <div className="inline-flex rounded-full bg-gray-100 p-1 text-xs font-medium text-gray-700">
                    ...
                    <button
                      type="button"
                      onClick={() => navigate('/dashboard/buyCredits?tab=subscription')}
                    ...
                    <button
                      type="button"
                      onClick={() => navigate('/dashboard/buyCredits?tab=lifetime')}
                ...
                {buyCreditsTab === 'credits' ? (
                  <div className="flex flex-col items-center">
                    <div className="flex items-center gap-4 text-xs font-medium text-gray-500 mb-8 w-fit bg-gray-50 rounded-full py-2 px-6">
                      <span className="flex items-center gap-1.5"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"/></svg>30-Day Money Back</span>
                      <span className="text-gray-300">|</span>
                      <span className="flex items-center gap-1.5"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>Credits Never Expire</span>
                      <span className="text-gray-300">|</span>
                      <span className="flex items-center gap-1.5"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1"/></svg>1 Credit = 1h Call →</span>
                    </div>
                    <div className="grid gap-6 md:grid-cols-3 max-w-4xl mx-auto w-full">
                      ...
                      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8 flex flex-col items-center text-center hover:border-emerald-500 transition-colors">
                        <div className="mb-2 text-emerald-500">
                          <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor" className="mx-auto block">
                            <circle cx="12" cy="6" r="2" />
                            <circle cx="12" cy="12" r="2" />
                            <circle cx="12" cy="18" r="2" />
                          </svg>
                        </div>
                        <h3 className="text-sm font-semibold text-emerald-500 mb-4">Basic</h3>
                        <div className="flex flex-col items-center justify-center mb-6">
                          <span className="text-3xl font-bold text-gray-900">₹2,650</span>
                          <span className="text-sm text-gray-500">($29.50)</span>
                        </div>
                        <p className="text-sm text-gray-500 mb-8 mt-auto">3 Call Credits</p>
                        <button type="button" className="w-full inline-flex items-center justify-center gap-2 rounded-full text-sm font-semibold px-4 py-3 bg-emerald-700 hover:bg-emerald-800 text-white transition-colors">
                          Get Credits <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 5l7 7m0 0l-7 7m7-7H3"/></svg>
                        </button>
                      </div>
                      ...
                      <div className="bg-emerald-700 rounded-xl shadow-md border-0 p-8 flex flex-col items-center text-center transform scale-105 z-10 relative">
                        <div className="mb-2 text-white">
                          <svg width="32" height="32" viewBox="0 0 24 24" fill="currentColor" className="mx-auto block text-yellow-400 opacity-90">
                        ...
                        <h3 className="text-sm font-semibold text-white mb-4">Plus</h3>
                        <div className="flex flex-col items-center justify-center mb-6">
                          <span className="text-4xl font-bold text-white">₹5,300</span>
                          <span className="text-sm text-green-100">($59.00)</span>
                        </div>
                        <p className="text-sm text-white mb-8 mt-auto font-medium">6 Call Credits + 2 Free</p>
                        <button type="button" className="w-full inline-flex items-center justify-center gap-2 rounded-full text-sm font-semibold px-4 py-3 bg-white hover:bg-gray-50 text-emerald-800 transition-colors">
                          Get Credits <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 5l7 7m0 0l-7 7m7-7H3"/></svg>
                        </button>
                      </div>
                      ...
                      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8 flex flex-col items-center text-center hover:border-emerald-500 transition-colors">
                        <div className="mb-2 flex items-center justify-center">
                          <svg width="32" height="32" viewBox="0 0 24 24" fill="currentColor" className="text-emerald-500 block">
                            <circle cx="6" cy="10" r="1.5" />
                            <circle cx="10" cy="10" r="1.5" />
                            <circle cx="14" cy="10" r="1.5" className="text-yellow-400" />
                            <circle cx="18" cy="10" r="1.5" className="text-yellow-400" />
                            <circle cx="6" cy="14" r="1.5" />
                            <circle cx="10" cy="14" r="1.5" />
                            <circle cx="14" cy="14" r="1.5" className="text-yellow-400" />
                            <circle cx="18" cy="14" r="1.5" className="text-yellow-400" />
                          </svg>
                        </div>
                        <h3 className="text-sm font-semibold text-emerald-500 mb-4">Pro</h3>
                        <div className="flex flex-col items-center justify-center mb-6">
                          <span className="text-3xl font-bold text-gray-900">₹7,950</span>
                          <span className="text-sm text-gray-500">($88.50)</span>
                        </div>
                        <p className="text-sm text-gray-500 mb-8 mt-auto">9 Call Credits + 6 Free</p>
                        <button type="button" className="w-full inline-flex items-center justify-center gap-2 rounded-full text-sm font-semibold px-4 py-3 bg-emerald-700 hover:bg-emerald-800 text-white transition-colors">
                          Get Credits <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 5l7 7m0 0l-7 7m7-7H3"/></svg>
                        </button>
                      </div>
                  ...
                  <div className="flex flex-col items-center">
                    <div className="flex items-center gap-4 text-xs font-medium text-gray-500 mb-8 w-fit bg-gray-50 rounded-full py-2 px-6">
                      <span className="flex items-center gap-1.5"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"/></svg>30-Day Money Back</span>
                      <span className="text-gray-300">|</span>
                      <span className="flex items-center gap-1.5"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>Unlimited Calls</span>
                      <span className="text-gray-300">|</span>
                      <span className="flex items-center gap-1.5"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636"/></svg>Cancel Anytime</span>
                    </div>
                    <div className="grid gap-6 md:grid-cols-2 max-w-3xl mx-auto w-full">
                      {/* Monthly */}
                      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8 flex flex-col items-center text-center hover:border-emerald-500 transition-colors">
                        <div className="mb-2 text-emerald-500">
                          <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="mx-auto block">
                        ...
                        <h3 className="text-lg font-semibold text-emerald-500 mb-4">Monthly</h3>
                        <div className="flex flex-col items-center justify-center mb-6">
                          <span className="text-4xl font-bold text-gray-900">₹6,730</span>
                          <span className="text-sm text-gray-500 mt-1">($74.90)</span>
                          <span className="text-xs text-gray-500 mt-1">per month</span>
                        </div>
                        <p className="text-sm text-gray-500 mb-8 mt-auto">Unlimited Calls</p>
                        <button type="button" className="w-full inline-flex items-center justify-center gap-2 rounded-full text-sm font-semibold px-4 py-3 bg-emerald-800 hover:bg-emerald-900 text-white transition-colors">
                      ...
                      <div className="bg-emerald-800 rounded-xl shadow-md border-0 p-8 flex flex-col items-center text-center transform scale-105 z-10 relative">
                        ...
                        <h3 className="text-lg font-semibold text-white mb-4">Yearly</h3>
                        <div className="flex flex-col items-center justify-center mb-6">
                          <span className="text-4xl font-bold text-white">₹20,190</span>
                          <span className="text-sm text-green-100 mt-1">($224.90)</span>
                          <span className="text-xs text-green-100 mt-1">per year</span>
                        </div>
                        <p className="text-sm text-white mb-8 mt-auto font-medium">Unlimited Calls • Save 75%</p>
                        <button type="button" className="w-full inline-flex items-center justify-center gap-2 rounded-full text-sm font-semibold px-4 py-3 bg-white hover:bg-gray-50 text-emerald-900 transition-colors">
                          Subscribe <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 5l7 7m0 0l-7 7m7-7H3"/></svg>
                        </button>
                      </div>
                    </div>
                  ...
                  <div className="flex flex-col items-center">
                    <div className="flex items-center gap-4 text-xs font-medium text-gray-500 mb-8 w-fit bg-gray-50 rounded-full py-2 px-6">
                      <span className="flex items-center gap-1.5"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"/></svg>30-Day Money Back</span>
                      <span className="text-gray-300">|</span>
                      <span className="flex items-center gap-1.5"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>Unlimited Calls Forever</span>
                      <span className="text-gray-300">|</span>
                      <span className="flex items-center gap-1.5"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"/></svg>One-Time Payment</span>
                    </div>
                    <div className="w-full max-w-sm mx-auto">
                      <div className="bg-emerald-800 rounded-xl shadow-md border-0 p-8 flex flex-col items-center text-center">
                        <div className="mb-2 text-white">
                          <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="mx-auto block text-white">
                        ...
                        <h3 className="text-lg font-semibold text-white mb-4">Lifetime</h3>
                        <div className="flex flex-col items-center justify-center mb-6">
                          <span className="text-4xl font-bold text-white">₹40,990</span>
                          <span className="text-sm text-green-100 mt-1">($449.90)</span>
                          <span className="text-xs text-green-100 mt-1">once</span>
                        </div>
                        <p className="text-sm text-white mb-8 mt-auto font-medium">Unlimited Calls Forever</p>
                        <button type="button" className="w-full inline-flex items-center justify-center gap-2 rounded-full text-sm font-semibold px-4 py-3 bg-white hover:bg-gray-50 text-emerald-900 transition-colors">
                          Get Lifetime <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 5l7 7m0 0l-7 7m7-7H3"/></svg>
                        </button>
                      </div>
                    </div>
                  </div>
            ...
            <div className="p-6 w-full min-w-0">
              ...
              {resumesList.length === 0 ? (
                ...
                <>
                  <ul className="divide-y divide-gray-200">
                    {resumesList.map((r) => (
                      <li key={r.id} className="flex items-center justify-between py-3">
                        ...
                        >
                          <span className="text-gray-900 text-sm font-medium truncate group-hover:text-primary-600 group-hover:underline">{r.title}</span>
                  ...
                  <p className="mt-10 text-center text-xs text-gray-400">A list of your Resumes.</p>
                </>
              )}
            </div>
            ...
            <div className="p-6 w-full min-w-0">
                ...
                <table className="w-full text-left text-sm">
                  ...
                  <tbody>
                    {sessionsLoading ? (
                      ...
                      <tr><td colSpan={7} className="py-8 text-center text-gray-500">No sessions yet.</td></tr>
                    ) : (
                      sessionsList.map((s) => (
                        (() => {
                          ...
                          const endsAtMs = s.endsAt ? Date.parse(s.endsAt) : NaN
                          const hasEndsAt = Number.isFinite(endsAtMs)
                          const remainingMs = hasEndsAt ? endsAtMs - Date.now() : NaN
                          const isEnded = status === 'ended' || (hasEndsAt && remainingMs <= 0)
                          ...
                          return (
                            <tr
                              key={s.id}
                              onClick={() => (isEnded ? setConversationModalSession(s) : setSessionForPlatformModal(s))}
                              className={rowClass}
                            >
                              <td className={`py-3 pr-4 font-medium ${isEnded ? 'text-gray-500' : 'text-gray-900'}`}>{s.title || '—'}</td>
                              <td className={`py-3 pr-4 ${isEnded ? 'text-gray-500' : 'text-gray-600'}`}>{s.description || '—'}</td>
                          <td className="py-3 pr-4">
                            <span className="text-gray-600">{endsInDisplay}</span>
                            {s.isFreeSession ? (
                              <span className="ml-1.5 inline-flex items-center rounded bg-gray-100 px-1.5 py-0.5 text-xs font-medium text-gray-700">Free</span>
                            ) : (
                              <span className="ml-1.5 inline-flex items-center rounded bg-gray-800 px-1.5 py-0.5 text-xs font-medium text-white">Credits: 0.5</span>
                            )}
                          </td>
                          <td className="py-3 pr-4 text-gray-600">{s.aiUsage}</td>
                          <td className="py-3 pr-4 text-gray-500">{new Date(s.createdAt).toLocaleDateString(undefined, { dateStyle: 'medium' })}</td>
                          <td className="py-3 text-right" onClick={(e) => e.stopPropagation()}>
                            <div className="flex items-center justify-end gap-1">
                              ...
                              <button
                                ...
                                disabled={isEnded}
                              >
                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.464 3.964z" /></svg>
                              </button>
                              <button type="button" className="p-1.5 rounded text-red-500 hover:bg-red-50" aria-label="Delete">
              ...
              {sessionsTotal > 0 && (
                <div className="mt-4 flex items-center justify-between text-sm text-gray-600">
                  <span>Page {sessionsPage} • Showing {(sessionsPage - 1) * 10 + 1}-{Math.min(sessionsPage * 10, sessionsTotal)} of {sessionsTotal}</span>
            ...
            <div className="p-8 max-w-5xl mx-auto">
              <div className="space-y-6 mt-2">
                ...
                <section className="bg-white rounded-xl border border-gray-200 p-5">
                  <h3 className="text-sm font-semibold text-gray-900 mb-2">Lifetime Access</h3>
                  <p className="text-xs text-gray-500 mb-3">No lifetime access purchased</p>
                  <button
                    type="button"
                    onClick={() => navigate('/dashboard/buyCredits?tab=lifetime')}
                    className="w-full inline-flex items-center justify-center gap-2 px-3 py-2 rounded-lg border border-gray-300 text-sm font-medium text-gray-700 hover:bg-gray-50"
                ...
                <section className="bg-white rounded-xl border border-gray-200 p-5">
                  ...
                  <p className="text-xs text-gray-600">
                    Deleting your account will permanently remove your data, credits, active subscriptions, and lifetime
                ...
                <div className="mt-6 space-y-4">
                  ...
                  {uploadSuccess ? (
                    <div className="py-6 text-center">
                      <p className="text-base font-medium text-green-600">{uploadSuccess}</p>
                    </div>
                  ) : uploadError ? (
                    <div className="py-4 text-center">
                      <p className="text-base font-medium text-red-600">{uploadError}</p>
                      <button
                        type="button"
                    ...
                    <>
                      <div className="flex items-center justify-center gap-2 rounded-xl border-2 border-dashed border-gray-300 bg-gray-50/50 py-4 px-4">
                        <span className="text-sm text-gray-600 truncate flex-1 min-w-0">{resumeFile.name}</span>
                        <button
                          type="button"
                      ...
                      <div className="flex gap-3 pt-2">
                        <button
                          type="button"
                          onClick={() => { setResumeModalStep('options'); handleRemoveResumeFile(); }}
                  ...
                  >
                    ...
                    {resumeFile && !fileSelected ? (
                      <p className="text-sm text-gray-600 text-center mb-3">Uploading... {uploadProgress}%</p>
                    ) : (
                      <>
                  ...
                  <div className="flex gap-3 pt-2">
                    ...
                    <button type="button" className="flex-1 rounded-lg bg-gray-900 text-white text-sm font-medium py-2.5 hover:bg-gray-800" disabled>
                      Upload Resume
                    </button>
                  </div>
                    </>
        ...
        {showCreateSessionModal && (
          <div className="fixed inset-0 z-40 flex items-center justify-center px-4">
            ...
            <div className="relative z-50 w-full max-w-lg rounded-2xl bg-white shadow-2xl border border-gray-200 overflow-hidden">
              ...
              {createSessionStep === 1 && (
                <div className="p-6">
                  <h2 className="text-lg font-semibold text-gray-900 flex items-center gap-2">
                    ...
                    {createSessionIsPaid ? 'New Session (0.5 credit)' : 'Free Session (10 min)'}
                  </h2>
                  <div className="mt-5 space-y-4">
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">
                        Company <span className="inline-flex items-center text-gray-400" title="Company name">ⓘ</span>
                      </label>
                      <input
                        type="text"
                        value={createSessionForm.company}
                        onChange={(e) => setCreateSessionForm((f) => ({ ...f, company: e.target.value }))}
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">
                        Job Description <span className="inline-flex items-center text-gray-400" title="Job description">ⓘ</span>
                      </label>
                      <textarea
                        value={createSessionForm.jobDescription}
                        onChange={(e) => setCreateSessionForm((f) => ({ ...f, jobDescription: e.target.value }))}
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">
                        Resume <span className="inline-flex items-center text-gray-400" title="Select resume">ⓘ</span> 📎
                      </label>
                      <div className="flex items-center gap-2">
                        <select
                          value={createSessionForm.resumeId}
                          onChange={(e) => setCreateSessionForm((f) => ({ ...f, resumeId: e.target.value }))}
                        ...
                        {createSessionForm.resumeId && (
                          <button
                            type="button"
                            onClick={() => setCreateSessionForm((f) => ({ ...f, resumeId: '' }))}
              ...
              {createSessionStep === 2 && (
                <div className="p-6">
                  <h2 className="text-lg font-semibold text-gray-900">Language & AI Settings</h2>
                  <div className="mt-5 space-y-4">
                    <div className="flex items-center justify-between gap-4">
                      <div className="flex-1">
                        <label className="block text-sm font-medium text-gray-700 mb-1">Language <span className="text-gray-400">ⓘ</span></label>
                        <select
                          value={createSessionForm.language}
                          onChange={(e) => setCreateSessionForm((f) => ({ ...f, language: e.target.value }))}
                      ...
                      <div className="flex-1">
                        <label className="block text-sm font-medium text-gray-700 mb-1">Simple Language <span className="text-gray-400">ⓘ</span></label>
                        <button
                          type="button"
                          role="switch"
                          aria-checked={createSessionForm.simpleLanguage}
                          onClick={() => setCreateSessionForm((f) => ({ ...f, simpleLanguage: !f.simpleLanguage }))}
                        ...
                        >
                          <span className={`inline-block h-5 w-5 transform rounded-full bg-white shadow transition translate-x-0.5 translate-y-0.5 ${createSessionForm.simpleLanguage ? 'translate-x-5' : 'translate-x-0'}`} />
                        </button>
                      </div>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Extra Context/Instructions <span className="text-gray-400">ⓘ</span></label>
                      <textarea
                        value={createSessionForm.extraContext}
                        onChange={(e) => setCreateSessionForm((f) => ({ ...f, extraContext: e.target.value }))}
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">AI Model <span className="text-gray-400">ⓘ</span></label>
                      <div className="flex items-center gap-2 rounded-lg border border-gray-300 px-3 py-2">
                        <span className="text-sm font-medium text-gray-900">GPT-4.1 Mini</span>
                        <span className="rounded bg-gray-800 px-1.5 py-0.5 text-xs font-medium text-white">Recommended</span>
                        <span className="rounded bg-gray-100 px-1.5 py-0.5 text-xs font-medium text-gray-700">Fast</span>
                      </div>
                    </div>
                  </div>
              ...
              {createSessionStep === 3 && (
                <div className="p-6">
                  <h2 className="text-lg font-semibold text-gray-900">Save Transcript</h2>
                  <div className="mt-5 space-y-4">
                    <div className="flex items-center justify-between">
                      <label className="text-sm font-medium text-gray-700">Save Transcript</label>
                      <button
                        type="button"
                        role="switch"
                        aria-checked={createSessionForm.saveTranscript}
                        onClick={() => setCreateSessionForm((f) => ({ ...f, saveTranscript: !f.saveTranscript }))}
                      ...
                      >
                        <span className={`inline-block h-5 w-5 transform rounded-full bg-white shadow transition translate-x-0.5 translate-y-0.5 ${createSessionForm.saveTranscript ? 'translate-x-5' : 'translate-x-0'}`} />
                      </button>
                    </div>
                    <p className="text-sm text-gray-600">
                      If you check this option, a transcript of the call session will be saved. You will be able to view the transcript in your dashboard after the call session has ended.
                    </p>
                    <div>
                      <p className="text-sm font-medium text-gray-700 mb-1">Legal Disclaimer</p>
                      <p className="text-sm text-gray-600">
                        You must comply with all applicable transcribing laws when using this transcription app. Many jurisdictions require consent from all parties being recorded. Transcribing without proper consent may be illegal.
                      </p>
                    </div>
                  </div>
                  <div className="mt-6 flex justify-between">
              ...
              {createSessionStep === 4 && (
                <div className="p-6">
                  {createSessionMode === 'update' ? (
                    <>
                      <h2 className="text-lg font-semibold text-gray-900">Ready to Update</h2>
                      <p className="mt-3 text-sm text-gray-600">
                        Review your changes and click &quot;Update Session&quot; to save the changes to your session.
                      </p>
                    </>
                  ) : (
                    <>
                      ...
                      {createSessionIsPaid ? (
                        <div className="mt-5 space-y-2 text-sm text-gray-700">
                          <p>💳 This session uses <strong>0.5 credit</strong> from your balance.</p>
                          <p>The timer will not start until you connect your screen sharing.</p>
                        </div>
                      ) : (
                        <div className="mt-5 space-y-2 text-sm text-gray-700">
                          <p>⏰ This is a 10 minute free session.</p>
                          <p>The timer will not start until you connect your screen sharing.</p>
                          <p>You won&apos;t be able to create another free session for the next 12 minutes.</p>
                        </div>
                      )}
                    </>
                  )}
                  <div className="mt-6 flex justify-between">
                    ...
                    <button
                      ...
                      onClick={createSessionMode === 'update' ? handleUpdateSession : handleCreateFreeSession}
                    ...
                    >
                      {createSessionMode === 'update'
                        ? (createSessionSubmitting ? 'Updating…' : 'Update Session')
                        : (createSessionSubmitting ? 'Creating…' : createSessionIsPaid ? 'Create Session' : 'Create Free Session')}
                    </button>
                  </div>
                </div>
              )}
              ...
              {createSessionMode !== 'update' && createSessionStep === 5 && (
                <div className="p-6">
                  ...
                  <p className="mt-1 text-sm text-gray-500">How would you like to connect to your call session?</p>
                  <div className="mt-5 space-y-3">
                    <button
                      type="button"
                      onClick={() => {
                        if (createdSessionIdForLaunch) launchDesktopApp(createdSessionIdForLaunch, createSessionForm.resumeId || undefined)
                    ...
                    >
                      ...
                      Desktop App
                      <span className="absolute right-3 rounded-full bg-green-500/90 px-2 py-0.5 text-xs font-medium text-white">Recommended</span>
                    </button>
                    <p className="text-center text-xs text-gray-400">Opens the app and saves conversation to this session. Install the app if it doesn’t open.</p>
              ...
              <div className="p-6">
                ...
                <div className="mt-5 space-y-3">
                  <button
                    type="button"
                    onClick={() => {
                      launchDesktopApp(sessionForPlatformModal.id, sessionForPlatformModal.resumeId)
            ...
            <div className="relative z-50 w-full max-w-2xl max-h-[85vh] rounded-2xl bg-white shadow-2xl border border-gray-200 overflow-hidden flex flex-col">
              <div className="flex items-center justify-between p-4 border-b border-gray-200">
                <h2 className="text-lg font-semibold text-gray-900">
                  Conversation {conversationModalSession.title && `· ${conversationModalSession.title}`}
                </h2>
                <button
                  type="button"
                  onClick={() => setConversationModalSession(null)}
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-3 min-h-0">
                {conversationMessagesLoading ? (
                  ...
                  <p className="text-sm text-gray-500 text-center py-8">No conversation yet. Start the desktop app and run a session to see the transcript here.</p>
                ) : (
                  conversationMessages.map((msg) => (
                    ...
                    >
                      <div className="font-medium text-xs opacity-80 mb-0.5">{msg.role}</div>
        ...
        {deleteResumeId && (
          <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
            ...
            <div className="relative z-50 w-full max-w-sm rounded-2xl bg-white shadow-2xl border border-gray-200 p-6">
              ...
              <h2 className="text-lg font-semibold text-gray-900">Delete Resume</h2>
              <p className="mt-3 text-sm text-gray-600">
